...
// import { Component, ElementRef, ViewChild, computed, effect, input, signal, inject } from '@angular/core';
// import { CommonModule } from '@angular/common';
// import { DomSanitizer, SafeResourceUrl } from '@angular/platform-browser';
...
//   // Securely construct the YouTube URL internally if needed
//   safeYouTubeUrl = computed<SafeResourceUrl | null>(() => {
//     if (!this.isYouTube()) return null;
//     // modestbranding and rel=0 keep users inside your ecosystem
//     const url = `https://www.youtube-nocookie.com/embed/${this.mediaToken()}?rel=0&modestbranding=1&controls=0`;
//     return this.sanitizer.bypassSecurityTrustResourceUrl(url);
//   });

//   // Construct secure stream URL for custom media
//   secureMediaStream = computed(() => {
//     if (this.isYouTube()) return null;
//     // Example: Using a backend endpoint that requires an HttpOnly cookie
//     return `/api/secure-stream/stream?token=${this.mediaToken()}`;
//   });

//   // --- ACTIONS ---
//   togglePlay() {
//     if (this.isYouTube()) return; // YT requires its own Iframe API for custom controls
...
//     // Advanced: Save progress to backend every 10 seconds
