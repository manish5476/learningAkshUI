  ...
  template: `
    <div class="card-wrapper">
      <a [routerLink]="['/courses', course?.slug || course?._id]"
         class="property-card"
         [attr.aria-label]="'View details for ' + (course?.title || 'Course')">
        ...
        <div class="content-wrapper">
          <div class="meta-row">
            ...
            <div class="rating-group">
              ...
              <span class="rating-value">{{ (course?.rating && course!.rating > 0) ? course!.rating.toFixed(1) : 'New' }}</span>
            </div>
          </div>
          ...
          <dl>
            ...
            <div class="title-block">
              ...
              <dd class="title-text">{{ course?.title || 'Untitled Course' }}</dd>
            </div>
          </dl>
...
export class PropertyCourseCardComponent {
  ...
  private _course!: Course | any; // Any allows for missing fields temporarily
  
  @Input({ required: true }) 
  set course(value: Course | any) {
    this._course = value;
    this.calculateDerivedProperties();
  }
  get course(): Course | any {
    return this._course;
  }

  private gradients = ['var(--theme-accent-gradient)', 'linear-gradient(135deg, var(--color-info-light) 0%, var(--color-info-dark) 100%)'];

  // Much stronger logic to handle populated vs unpopulated instructors
  showEditButton$: Observable<boolean> = this.auth.currentUser$.pipe(
    map((user: any) => {
      if (!user) return false;
      
      // The instructor field might be an object { _id: '...' } OR just a string ID
      const courseInstructorId = this._course?.instructor?._id || this._course?.instructor;
      
      // Ensure we check both user._id and user.id 
      const currentUserId = user._id || user.id;

      return user.role === 'admin' || (user.role === 'instructor' && currentUserId === courseInstructorId);
    })
  );
  ...
  private calculateDerivedProperties(): void {
    ...
    // 2. Discount Math (Safe check for missing properties)
    ...
    // 3. Duration Math
    const minutes = this._course.totalDuration || 0;
    if (!minutes) {
