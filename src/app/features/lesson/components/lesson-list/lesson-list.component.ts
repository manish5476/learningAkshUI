...
export class LessonListComponent implements OnInit {
    ...
    this.lessonService.getSectionLessons(sectionId)
      ...
      .subscribe({
        ...
        error: (error: any) => {
          ...
          this.messageService.showError('Failed to load lessons'         );
          this.loading.set(false);
        }
      });
  }
    ...
    this.lessonService.reorderLessons(this.sectionId(), orderedIds)
      ...
      .subscribe({
        ...
        error: (error: any) => {
          ...
          this.messageService.showError( 'Failed to reorder lessons'
          );
        ...
        this.lessonService.deleteLesson(lesson._id)
          ...
          .subscribe({
            ...
            error: (error: any) => {
              ...
              this.messageService.showError(error.error?.message || 'Failed to delete lesson');
            }
          });
      }
    });
  ...
  onLessonSaved(lesson: any): void {
    this.messageService.showSuccess( `Lesson ${this.selectedLessonId() ? 'updated' : 'created'} successfully`
    );
    this.closeDialog();
    this.loadLessons(this.sectionId());
  }
