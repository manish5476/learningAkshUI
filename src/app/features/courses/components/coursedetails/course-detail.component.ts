...
export class CourseDetailComponent implements OnInit {
  ...
  private loadCourse(identifier: string): void {
    ...
    request$.pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (res: any) => {
        ...
        const c = this.isInstructorMode() ? payload?.data : payload?.course;
        
        this.course.set(c);
        this.sections.set(payload?.sections || []);
  ...
  private calculateMetrics(courseData: Course | undefined): void {
    ...
    this.completionRate.set(Math.round(Math.random() * 100)); // Mock logic
  ...
  editPricing(): void {
    const c = this.course();
    if (c) {
      this.router.navigate(['/courses/instructor', c._id, 'edit'], { fragment: 'pricing' });
    }
  }
  ...
  unpublishCourse(): void {
    ...
    this.courseService.publish(c._id) // Assuming API toggles based on current state or payload
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
