import { Component, OnInit, inject, DestroyRef, input, signal, effect, ViewChild, ElementRef, AfterViewChecked } from '@angular/core';
...
export class CourseDiscussionComponent implements OnInit {
  ...
  scrollToBottom(): void {
    ...
    }, 100); // Small delay to let Angular render the new DOM element
  }
        ...
        next: (res) => {
          ...
          if (this.selectedDiscussion()) {
            ...
            if (updated) {
              ...
              this.backToList(); // OP was deleted
            }
          }
        },
        error: (err) => {
  ...
  viewThread(discussion: Discussion): void {
    ...
    this.scrollToBottom(); // Scroll to bottom when opening thread
  }
    ...
    this.discussionService.replyToDiscussion(this.courseId(), this.selectedDiscussion()!._id, this.replyForm.value.content)
      ...
      .subscribe({
        next: () => {
          ...
          this.loadDiscussions(true); // Tell it to scroll to bottom after load
        }
      });
  }
  ...
  deleteReply(replyId: string): void {
    if (confirm('Are you sure you want to delete this reply?')) {
      // NOTE: You need a delete reply route in your backend for this to work!
      // Example: this.discussionService.deleteReply(this.courseId(), this.selectedDiscussion()._id, replyId).subscribe(...)
      
      // For now, doing an optimistic UI mock assuming you add the backend route later:
      const currentThread = this.selectedDiscussion();
      if(currentThread && currentThread.replies) {
         currentThread.replies = currentThread.replies.filter(r => r._id !== replyId);
         this.selectedDiscussion.set({...currentThread});
      }
    }
  }
  ...
  toggleLike(event: Event, item: any, type: 'discussion' | 'reply'): void {
    ...
    if (hasLiked) item.likes = item.likes.filter((id: string) => id !== userId);
