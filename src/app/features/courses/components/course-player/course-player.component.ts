        ...
        .pipe(
          ...
          catchError((err) => {
            if (err.status === 403) {
              ...
              this.messageService.showError( 'Could not load lesson content'               );
            }
            return of(null);
          })
        )
...
if (result?.passed) {
  this.messageService.showSuccess(
    `Quiz Passed! You scored ${result.score}% - Great job!`
  );
} else {
  this.messageService.showInfo?.(
    `Quiz Completed. You scored ${result?.score ?? 0}% - Keep practicing!`
  );
 }
  }
