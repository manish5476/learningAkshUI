...
export class SignupComponent implements OnInit, OnDestroy {
  ...
  passwordStrength: PasswordStrength = { score: 0, label: '', cssClass: '' };

  // ── Static data ──────────────────────────────────────────
  readonly features = [
    { emoji: '📚', iconClass: 'fi-orange', title: '5,000+ Video Lectures', desc: 'UPSC, SSC, Banking, Railway & State PSC — all in one platform' },
    { emoji: '✅', iconClass: 'fi-green',  title: 'Daily Current Affairs + MCQ Tests', desc: 'Updated every morning with PIB, The Hindu, IE analysis' },
    { emoji: '🏆', iconClass: 'fi-gold',   title: 'Previous Year Papers (PYQ)', desc: 'Last 15 years solved with detailed explanations' },
    { emoji: '🤝', iconClass: 'fi-blue',   title: 'Mentorship by IAS / IPS Officers', desc: 'Live doubt sessions & 1:1 interview guidance' }
  ];

  readonly examOptions: string[] = [
    'UPSC IAS / IPS', 'SSC CGL / CHSL', 'IBPS PO / Clerk',
    'RRB NTPC / JE', 'State PSC', 'NDA / CDS',
    'CUET UG / PG', 'Teaching (TET/CTET)'
  ];

  readonly subjectOptions: string[] = [
    'History', 'Geography', 'Polity', 'Economics',
    'Maths', 'English', 'Science & Tech', 'Current Affairs'
  ];
  ...
  // ── Selected tags ────────────────────────────────────────
  selectedExams:    Set<string> = new Set();
  selectedSubjects: Set<string> = new Set();

  // Progress steps (1-based)
  currentStep = 1;
  ...
  private watchStepProgress(): void {
    this.subscriptions.push(
      this.signupForm.valueChanges.subscribe(() => {
        ...
        if (password && password.length >= 8) { this.currentStep = 3; }
        else if (email && /\S+@\S+\.\S+/.test(email)) { this.currentStep = 2; }
        else if (firstName) { this.currentStep = 1; }
      })
    );
  }
  ...
  private calcStrength(pw: string): PasswordStrength {
    ...
    const map: Record<number, { label: string; cssClass: string }> = {
      0: { label: '',        cssClass: '' },
      1: { label: 'Weak',    cssClass: 'weak' },
      2: { label: 'Fair',    cssClass: 'fair' },
      3: { label: 'Good',    cssClass: 'good' },
      4: { label: 'Strong',  cssClass: 'strong' }
    };
    return { score, ...map[score] };
  }

  // ── Role ─────────────────────────────────────────────────
  get role(): UserRole { return this.signupForm.get('role')!.value; }

  setRole(role: UserRole): void {
    this.signupForm.patchValue({ role });
    // Clear instructor-only / student-only fields on switch
  ...
  toggleExam(exam: string): void {
    this.selectedExams.has(exam)
      ? this.selectedExams.delete(exam)
      : this.selectedExams.add(exam);
  }

  toggleSubject(subj: string): void {
    this.selectedSubjects.has(subj)
      ? this.selectedSubjects.delete(subj)
      : this.selectedSubjects.add(subj);
  }

  isExamSelected(exam: string): boolean    { return this.selectedExams.has(exam); }
  isSubjectSelected(subj: string): boolean { return this.selectedSubjects.has(subj); }
  ...
  onSubmit(): void {
    ...
    const payload = {
      ...
      targetExams:  [...this.selectedExams],
